...
ACNNTO::ACNNTO(int32_t _population_size, int32_t _max_genomes, const vector<string> &_input_parameter_names, const vector<string> &_output_parameter_names, const map<string,double> &_normalize_mins, const map<string,double> &_normalize_maxs, int32_t _bp_iterations, double _learning_rate, bool _use_high_threshold, double _high_threshold, bool _use_low_threshold, double _low_threshold, string _output_directory, int32_t _ants, int32_t _hidden_layers_depth, int32_t _hidden_layer_nodes, double _pheromone_decay_parameter, double _pheromone_update_strength, double _pheromone_heuristic, int32_t _max_recurrent_depth, double _weight_reg_parameter, string _reward_type, string _norm, bool _bias_forward_paths, bool _bias_edges, bool _use_pheromone_weight_update, bool _use_edge_inherited_weights, bool _use_fitness_weight_update, bool _use_two_ants_types, double _const_phi ) : population_size(_population_size), max_genomes(_max_genomes), number_inputs(_input_parameter_names.size()), number_outputs(_output_parameter_names.size()), bp_iterations(_bp_iterations), learning_rate(_learning_rate), use_high_threshold(_use_high_threshold), high_threshold(_high_threshold), use_low_threshold(_use_low_threshold), low_threshold(_low_threshold), output_directory(_output_directory), ants(_ants), hidden_layers_depth(_hidden_layers_depth), hidden_layer_nodes(_hidden_layer_nodes), pheromone_decay_parameter(_pheromone_decay_parameter), pheromone_update_strength(_pheromone_update_strength), pheromone_heuristic(_pheromone_heuristic), max_recurrent_depth(_max_recurrent_depth), weight_reg_parameter(_weight_reg_parameter), reward_type(_reward_type), norm(_norm), bias_forward_paths(_bias_forward_paths), bias_edges(_bias_edges), use_pheromone_weight_update(_use_pheromone_weight_update), use_edge_inherited_weights(_use_edge_inherited_weights), use_fitness_weight_update(_use_fitness_weight_update), use_two_ants_types(_use_two_ants_types), const_phi(_const_phi) {
...
RNN_Genome* ACNNTO::explorer_social_ants_march() {
    ...
    while (1) {
        ...
        for (int ant=0; ant<ants; ant++) {
            ...
            pheromone_line = pick_line( social_colony[-1]->pheromone_lines ) ; //begine with imaginary node
