...
int init_result(RESULT& result, void*& data) {
    ...
    if (retval) {
        log_messages.printf(
            MSG_CRITICAL, "[RESULT#%ld %s] check_set: can't get output filenames\n", result.id, result.name
        );
        return retval;
    }
    ...
    if (files.size() > 1) {
        log_messages.printf(
            MSG_CRITICAL, "[RESULT#%ld %s] had more than one output file: %zu\n", result.id, result.name, files.size()
        );
    ...
    if (fi.no_validate) {
        log_messages.printf(
            MSG_CRITICAL, "[RESULT#%ld %s] had file set to no validate: %s\n", result.id, result.name, fi.path.c_str()
        );
        exit(1);
        // continue;
    }
    ...
    try {
        ...
        if (line.compare(EXACT_VERSION_STR) != 0) {
            log_messages.printf(
                MSG_CRITICAL, "[RESULT#%ld %s] get_data_from_result: invalid version\n", result.id, result.name
            );
            log_messages.printf(
                MSG_CRITICAL, "     file version was: '%s', requires '%s'\n", line.c_str(), EXACT_VERSION_STR
            );
            return 1;
        }
        ...
        if (best_error <= 0 || isnan(best_error) || isinf(best_error)) {
            log_messages.printf(
                MSG_CRITICAL, "[RESULT#%ld %s] get_data_from_result: invalid best_error\n", result.id, result.name
            );
        ...
        if (generalizability_error <= 0 || isnan(generalizability_error) || isinf(generalizability_error)) {
            log_messages.printf(
                MSG_CRITICAL, "[RESULT#%ld %s] get_data_from_result: invalid generalizability error\n", result.id,
                result.name
            );
        ...
        if (test_error <= 0 || isnan(test_error) || isinf(test_error)) {
            log_messages.printf(
                MSG_CRITICAL, "[RESULT#%ld %s] get_data_from_result: invalid test error\n", result.id, result.name
            );
        ...
        log_messages.printf(
            MSG_CRITICAL, "[RESULT#%ld %s] get_data_from_result: could not open file for result\n", result.id,
            result.name
        );
...
int compare_results(RESULT& r1, void* data1, RESULT const& r2, void* data2, bool& match) {
    ...
    if (f1->file_contents.compare(f2->file_contents) == 0) {
        ...
        match = false;
        log_messages.printf(
            MSG_CRITICAL, "[RESULT#%ld %s] and [RESULT#%ld %s] failed sets had different file contents.\n", r1.id,
