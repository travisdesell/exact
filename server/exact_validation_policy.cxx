  ...
  if (f1->file_contents.compare(f2->file_contents) == 0) {
    ...
    if (fabs(best_error1 - best_error2) < min_best_error_difference) {
      log_messages.printf(MSG_CRITICAL,
                          "[RESULT#%ld %s] and [RESULT#%ld %s] match because best_error difference is close enough "
                          "(less than %lf): %lf - %lf = %lf.\n",
                          r1.id, r1.name, r2.id, r2.name, min_best_error_difference, best_error1, best_error2,
                          fabs(best_error1 - best_error2));
      match = true;
    } else {
      log_messages.printf(MSG_CRITICAL,
                          "[RESULT#%ld %s] and [RESULT#%ld %s] DO NOT MATCH because best_error difference is close "
                          "enough (greater than %lf): %lf - %lf = %lf.\n",
                          r1.id, r1.name, r2.id, r2.name, min_best_error_difference, best_error1, best_error2,
                          fabs(best_error1 - best_error2));
      match = false;
    }
