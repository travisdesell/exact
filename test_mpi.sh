...
let np=$n_nodes*$node_size*2

for i in $(seq 0 39); do
  mkdir -p $results_dir/$i

  export OMPI_MCA_mpi_yield_when_idle=1 && srun -n $np $1/mpi/examm_mpi \
    --training_filenames ./datasets/2018_coal/burner_[0].csv --test_filenames \
    ./datasets/2018_coal/burner_1[0].csv \
    --time_offset 1 \
