...
int main(int argc, char **argv) {
  ...
  if (label_type.compare("POINT") == 0) {
    ...
    point_query << "SELECT point_id, cx, cy FROM `points` WHERE mosaic_id = " << mosaic_id
    ...
    job_query << "INSERT INTO jobs SET owner_id = " << owner_id << ", mosaic_id = " << mosaic_id
              << ", label_id = " << label_id << ", name = '" << job_name << "'";
    ...
    for (uint32_t i = 0; i < point_mosaic_images.get_number_large_images(); i++) {
      ...
      prediction_query << "INSERT INTO prediction SET job_id = " << job_id << ", owner_id = " << owner_id
                       << ", mosaic_id = " << mosaic_id << ", label_id = " << label_id << ", mark_id = " << point_ids[i]
    ...
    line_query << "SELECT line_id, x1, y1, x2, y2 FROM `lines` WHERE mosaic_id = " << mosaic_id
               << " AND label_id = " << label_id << endl;
    ...
    for (uint32_t i = 0; i < line_mosaic_images.get_number_large_images(); i++) {
      ...
      ostringstream prediction_query;
      prediction_query << "INSERT INTO prediction SET job_id = " << job_id << ", owner_id = " << owner_id
                       << ", mosaic_id = " << mosaic_id << ", label_id = " << label_id << ", mark_id = " << line_ids[i]
      ...
      ostringstream output_filename;
      output_filename << output_directory << job_id << "/line_" << mosaic_id << "_" << label_id << "_" << line_ids[i]
                      << ".png";
