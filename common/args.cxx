...
Argument::Argument(string name, string flag, string description, bool required, arg_type ty, data default_value) : name(name), flag(flag), description(description), required(required), default_value(default_value), ty(ty), value(default_value), parsed(false) {}
    ...
    case INT_LIST:
      {
        ...
        for (int i = 0; i < (int) values.size(); i++)
          ns.push_back(try_parse_int(values[i]));
        value = move(ns);
      }
      break;
    case STRING_LIST:
    ...
    case DOUBLE_LIST:
      {
        vector<double> ns;
        for (int i = 0; i < (int) values.size(); i++)
          ns.push_back(try_parse_double(values[i]));
        value = move(ns);
      }
      break;
  }
...
EnumArgument *EnumArgument::clone() {
...
void EnumArgument::accept(vector<string> values) {
  ...
  if (ty == INT) { 
    ...
    if (values.size() == 0)
      Log::warning("Found zero arguments for flag %s\n", flag.c_str());

    for (auto it = values.begin(); it != values.end(); it++)
      enum_values.push_back(find(*it));

    value = enum_values;
  } else {
    Log::fatal("EnumArgument %s should have type INT or INT_LIST. Change the default argument to a int or vector<int>.\n", flag.c_str());
  }
...
int EnumArgument::find(const string &s) {
  ...
  if (it == data_map.end()) {
    Log::fatal("Invalid enum value %s supplied for flag %s. Valid values are %s. Aborting. \n", s.c_str(), flag.c_str(), to_string().c_str());
    exit(1);
  }
  return it->second;
}

ConstrainedArgument::ConstrainedArgument(string name, string flag, string description, function<bool(Argument::data &)> constraint,
                      bool required, arg_type ty, data default_value)
  : Argument(name, flag, description, required, ty, default_value), constraint(constraint) {}

ConstrainedArgument::~ConstrainedArgument() {}

ConstrainedArgument *ConstrainedArgument::clone() {
  return new ConstrainedArgument(*this);
}
...
ArgumentSet::ArgumentSet(string name, initializer_list<Argument *> initializers, function<bool (ArgumentSet &)> validate) : name(name), validate(validate) {
...
ArgumentSet::ArgumentSet(string name, map<string, unique_ptr<Argument>> args, function<bool (ArgumentSet &)> validate) : name(name), args(move(args)), validate(validate) {}
...
ArgumentSet ArgumentSet::operator+(const ArgumentSet &other) {
  ...
  for (const auto &it : this->args)
    new_args.emplace(it.first, unique_ptr<Argument>(it.second->clone()));
  ...
  auto new_validate = [=](ArgumentSet &as) {
    return v_copy(as) && other_v_copy(as);
  };
...
void ArgumentSet::parse() {
  ...
  if (!validate(*this)) {
    Log::fatal("Failed to validate argument set %s. Aborting.\n", name.c_str());
  }
}
