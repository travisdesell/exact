...
class LogFile {
 private:
  static const int32_t BUFFER_LENGTH = 1024 * 4;
...
class Log {
 public:
   ...
   * Messages will be written to the standard output log if their type is <=
   * message_level.
   */
  ...
  /**
   *  The MPI process rank for this Log instance. Set to -1 if not specified or
   * not using MPI.
   */
  static int32_t process_rank;

  /**
   * Defaults at -1, when set to a process rank (>= 0) the Log will only print
   * messages if its rank is the same as the restricted rank. This is useful,
   * for allowing initialization messages to only be printed out by a single
   * process.
   */
   ...
   * will be written to.
   */
  static unordered_map<string, shared_ptr<LogFile>, LogHasher> output_files;
  // static unordered_map<string, shared_ptr<LogFile>> output_files;
  // static map<string, shared_ptr<LogFile>, LogComparator> output_files;
  // static map<string, shared_ptr<LogFile>> output_files;

  /**
   * Thread local map of string to output file; used so the lock to grab
   * output_files may not have to be grabbed.
   */
   ...
   * The Log::set_thread_id(int32_t) mehod needs write access so it will use the
   * std::shared_mutex::lock()x and std::shared_mutex()::unlock() mechanism,
   * while the Log::get_thread_id() only needs read access and can used the
   * std::shared_mutex::lock_shared() and std::shared_mutex::unlock_shared()
   * methods.
   */
  static shared_mutex output_files_mutex;

  /**
   * Potentially writes the message to either standard output or the log file if
   * the message level is high enough.
   *
   * \param print_header specifies if the header to the message should be
   * printed out \param message_level the level of the message to potentially be
   * printed out \param message_type a string representation of this message
   ...
   * class. Needs to be called at the beginning of the main method of any
   * programming using the Log class. Will register the following command line
  ...
  /**
   * \param a string representation of the message level, either as text (e.g.,
   * "INFO") or as a number (e.g., 4)
   *
   * \return the message level as an int8_t (i.e., one of the message level
   * constants)
   */
  ...
  /**
   * Specifies which MPI process to allow messages from. A value < 0 will allow
   *messages from any rank. \param _restricted_rank is the MPI rank to only
   *print messages from
   */
  static void restrict_to_rank(int32_t _restricted_rank);

  /**
   * Clears the MPI process rank restriction from messages allowing any process
   * to write to the log (sets Log::restricted_rank back to -1).
   */
   ...
   *
   * \return true if either the file or standard output level is greater than or
