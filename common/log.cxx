...
// inline thread_local unordered_map<string, shared_ptr<LogFile>>
// output_files_local(32); inline thread_local thread::id Log::thread_id =
// std::this_thread::get_id(); inline thread_local shared_ptr<LogFile>
// Log::current_log_file(nullptr); inline thread_local string
// human_readable_id("DEFAULT");
...
LogFile::~LogFile() {
  // locking is not necessary, since this deconstructor is only called by
  // shared_ptr when the last reference is dropped. log_file->file_mutex.lock();
  file.flush();
  file.close();
...
void Log::initialize(const vector<string> &arguments) {
  ...
  // cerr << "std_message_level: " << std_message_level << ",
  // file_message_level: " << file_message_level << endl;
...
void Log::set_id(string human_readable_id) {
  ...
  // cerr << "setting thread id " << id << " to human readable id: '" <<
  // human_readable_id << "'" << endl;
...
void Log::write_message(bool print_header, int8_t message_level, const char *message_type, const char *format,
  ...
  if (!Log::current_log_file) {
    cerr << "ERROR: could not write message from thread '" << Log::thread_id
         << "' because it did not have a human readable id assigned (please "
            "use the Log::set_id(string) function before writing to the Log on "
            "any thread)."
         << endl;
    cerr << "message:" << endl;
