...
float random_0_1(minstd_rand0 &generator) {
  return ((float)generator() - (float)generator.min()) /
         ((float)generator.max() - (float)generator.min());
}

void fisher_yates_shuffle(minstd_rand0 &generator, vector<int> &v) {
  for (int32_t i = v.size() - 1; i > 0; i--) {
    float t = ((float)generator() - (float)generator.min()) /
              ((float)generator.max() - (float)generator.min());
    t *= (float)i - 1.0;
...
float NormalDistribution::random(minstd_rand0 &generator, float mu,
                                 float sigma) {
  ...
  float u1, u2;
  do {
    u1 = ((float)generator() - (float)generator.min()) /
         ((float)generator.max() - (float)generator.min());
    u2 = ((float)generator() - (float)generator.min()) /
         ((float)generator.max() - (float)generator.min());
  } while (u1 <= epsilon);
...
ostream &operator<<(ostream &os,
  ...
  os << normal_distribution.generate << " " << setprecision(15)
     << normal_distribution.z0 << " " << setprecision(15)
...
istream &operator>>(istream &is, NormalDistribution &normal_distribution) {
  is >> normal_distribution.generate >> normal_distribution.z0 >>
      normal_distribution.z1;
