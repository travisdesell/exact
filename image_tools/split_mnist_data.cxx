...
uint32_t fread_uint32_t(ifstream &file, string filename, const char *name, uint32_t expected_value) {
  ...
  if (value != expected_value) {
    cerr << "Error reading from image file '" << filename << "'. Invalid format, " << name << " != " << expected_value
         << endl;
    exit(1);
  }
...
int main(int argc, char **argv) {
  if (argc != 7) {
    ...
    cerr << "usage: " << endl;
    cerr << "    " << argv[0]
         << " <mnist image file> <mnist label file> <output training file> <output validation file> <expected number "
            "of images> <validation images per label"
         << endl;
    exit(1);
  }
  ...
  cout << "validation file '" << output_filename_validation << " will have " << images_per_label << " images per label."
       << endl;
  ...
  for (uint32_t i = 0; i < number_labels; i++) { cout << "\timages[" << i << "].size(): " << images[i].size() << endl; }
