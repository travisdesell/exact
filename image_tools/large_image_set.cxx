...
LargeImage::LargeImage(
    ifstream& infile, int _number_subimages, int _channels, int _width, int _height, int _padding, int _classification,
    const LargeImages* _images
) {
...
void LargeImage::draw_tiff(string filename) const {
    ...
    if ((image_s = TIFFWriteEncodedStrip(
             output_image, 0, values, 3 * (width + (padding * 2)) * (height + (padding * 2)) * sizeof(int8_t)
         ))
        == -1) {
...
void LargeImage::draw_tiff_alpha(string filename) const {
    ...
    tsize_t image_s;
    if ((image_s = TIFFWriteEncodedStrip(
             output_image, 0, values, (width + (padding * 2)) * (height + (padding * 2)) * sizeof(int8_t)
         ))
        == -1) {
        std::cerr << "Unable to write tif file '" << filename << "'" << endl;
    } else {
...
void LargeImage::draw_tiff_4channel(string filename) const {
    ...
    tsize_t image_s;
    if ((image_s = TIFFWriteEncodedStrip(
             output_image, 0, values, samples_per_pixel * (width + (padding * 2)) * (height + (padding * 2))
         ))
        == -1) {
        std::cerr << "Unable to write tif file '" << filename << "'" << endl;
    } else {
    ...
    if ((dir = opendir(directory.c_str())) != NULL) {
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            vector<vector<vector<uint8_t> > > pixels(
                channels, vector<vector<uint8_t> >(height, vector<uint8_t>(width, 0))
            );
...
LargeImages::LargeImages(
    string _filename, int _padding, int _subimage_height, int _subimage_width, const vector<float>& _channel_avg,
