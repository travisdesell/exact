...
void process_dir(string dir_name, int depth) {
  ...
  if ((dir = opendir(dir_name.c_str())) != NULL) {
    ...
    // cerr << "ERROR: could not open the directory '" << dir_name << "'" <<
    // endl;
  }
}
...
int main(int argc, char **argv) {
  ...
  cout << "search_name,slice_name,best_mse,best_mae,n_edges,n_rec_edges,n_"
          "nodes,n_ff,n_lstm,n_ugrnn,n_delta,n_mgu,n_gru"
       << endl;
  ...
  for (auto i = genome_map.begin(); i != genome_map.end(); i++) {
    ...
    for (auto j = i->second.begin(); j != i->second.end(); j++) {
      ...
      /*
         cout << i->first << ",min," << mse.min() << "," << mae.min() << "," <<
         edge.min() << "," << rec_edge.min() << "," << node.min() << "," <<
         ff.min() << "," << lstm.min() << "," << ugrnn.min() << "," <<
         delta.min() << "," << mgu.min() << "," << gru.min() << endl; cout <<
         i->first << ",avg," << mse.avg() << "," << mae.avg() << "," <<
         edge.avg() << "," << rec_edge.avg() << "," << node.avg() << "," <<
         ff.avg() << "," << lstm.avg() << "," << ugrnn.avg() << "," <<
         delta.avg() << "," << mgu.avg() << "," << gru.avg() << endl; cout <<
         i->first << ",max," << mse.max() << "," << mae.max() << "," <<
         edge.max() << "," << rec_edge.max() << "," << node.max() << "," <<
         ff.max() << "," << lstm.max() << "," << ugrnn.max() << "," <<
         delta.max() << "," << mgu.max() << "," << gru.max() << endl; cout <<
         i->first << ",stddev," << mse.stddev() << "," << mae.stddev() << "," <<
         edge.stddev() << "," << rec_edge.stddev() << "," << node.stddev() <<
         "," << ff.stddev() << "," << lstm.stddev() << "," << ugrnn.stddev() <<
         "," << delta.stddev() << "," << mgu.stddev() << "," << gru.stddev() <<
         endl; cout << i->first << ",mse correlation," << mse.correlate(mse) <<
         "," << mae.correlate(mse) << "," << edge.correlate(mse) << "," <<
         rec_edge.correlate(mse) << "," << node.correlate(mse) << "," <<
         ff.correlate(mse) << "," << lstm.correlate(mse) << "," <<
         ugrnn.correlate(mse) << "," << delta.correlate(mse) << "," <<
         mgu.correlate(mse) << "," << gru.correlate(mse) << endl; cout <<
         i->first << ",mae correlation," << mse.correlate(mae) << "," <<
         mae.correlate(mae) << "," << edge.correlate(mae) << "," <<
         rec_edge.correlate(mae) << "," << node.correlate(mae) << "," <<
         ff.correlate(mae) << "," << lstm.correlate(mae) << "," <<
         ugrnn.correlate(mae) << "," << delta.correlate(mae) << "," <<
      ...
      if ((subdir = opendir(sub_dir_name.c_str())) != NULL) {
        while ((ent = readdir(subdir)) != NULL) {
          if (strlen(ent->d_name) > 4 && extension_is(ent->d_name, ".csv")) {
            ...
            // run_statistics.push_back(new RunStatistics(sweep_directory + "_"
            // + search_type, mse, mae));
          } else {
  ...
  cout << "\\caption{\\label{table:consolidated_rankings} Hidden node count "
          "correlations for EXAMM runs evolving all memory neuron types.}"
       << endl;
  ...
  for (int i = 0; i < output_types.size(); i++) {
    ...
    cout << "\\hline" << endl;
    cout << "\\multicolumn{2}{|c||}{{\\bf Best Case}} & "
            "\\multicolumn{2}{|c||}{{\\bf Avg. Case}} & "
    ...
    cout << "\\hline" << endl;
    cout << "\\end{tabular}" << endl;
    cout << "\\caption{\\label{table:consolidated_rankings} Rankings for all "
            "the EXAMM run types predicting "
         << output_types[i] << ".}" << endl;
    cout << "\\end{scriptsize}" << endl;
    cout << "\\end{table}" << endl;
  }
  ...
  cout << "\\hline" << endl;
  cout << "\\end{tabular}" << endl;
  cout << "\\caption{\\label{table:consolidated_rankings} Combined rankings "
          "for all EXAMM run types.}"
       << endl;
  cout << "\\end{scriptsize}" << endl;
  cout << "\\end{table}" << endl;
