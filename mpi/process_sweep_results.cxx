...
int main(int argc, char** argv) {
    ...
    cout << "search_name,slice_name,best_mse,best_mae,n_edges,n_rec_edges,n_nodes,n_ff,n_lstm,n_ugrnn,n_delta,n_mgu,n_"
            "gru"
         << endl;
        ...
        for (auto j = i->second.begin(); j != i->second.end(); j++) {
            ...
            for (auto k = j->second.begin(); k != j->second.end(); k++) {
                ...
                cout << i->first << "," << j->first << "," << genome->get_best_validation_mse() << ","
                     ...
                     << genome->get_enabled_node_count(DELTA_NODE) << "," << genome->get_enabled_node_count(MGU_NODE)
            ...
            /*
               cout << i->first << ",min," << mse.min() << "," << mae.min() << "," << edge.min() << "," <<
               rec_edge.min() << "," << node.min() << "," << ff.min() << "," << lstm.min() << "," << ugrnn.min() << ","
               << delta.min() << "," << mgu.min() << "," << gru.min() << endl; cout << i->first << ",avg," << mse.avg()
               << "," << mae.avg() << "," << edge.avg() << "," << rec_edge.avg() << "," << node.avg() << "," << ff.avg()
               << "," << lstm.avg() << "," << ugrnn.avg() << "," << delta.avg() << "," << mgu.avg() << "," << gru.avg()
               << endl; cout << i->first << ",max," << mse.max() << "," << mae.max() << "," << edge.max() << "," <<
               rec_edge.max() << "," << node.max() << "," << ff.max() << "," << lstm.max() << "," << ugrnn.max() << ","
               << delta.max() << "," << mgu.max() << "," << gru.max() << endl; cout << i->first << ",stddev," <<
               mse.stddev() << "," << mae.stddev() << "," << edge.stddev() << "," << rec_edge.stddev() << "," <<
               node.stddev() << "," << ff.stddev() << "," << lstm.stddev() << "," << ugrnn.stddev() << "," <<
               delta.stddev() << "," << mgu.stddev() << "," << gru.stddev() << endl; cout << i->first << ",mse
               correlation," << mse.correlate(mse) << "," << mae.correlate(mse) << "," << edge.correlate(mse) << "," <<
               rec_edge.correlate(mse) << "," << node.correlate(mse) << "," << ff.correlate(mse) << "," <<
               lstm.correlate(mse) << "," << ugrnn.correlate(mse) << "," << delta.correlate(mse) << "," <<
               mgu.correlate(mse) << "," << gru.correlate(mse) << endl; cout << i->first << ",mae correlation," <<
               mse.correlate(mae) << "," << mae.correlate(mae) << "," << edge.correlate(mae) << "," <<
               rec_edge.correlate(mae) << "," << node.correlate(mae) << "," << ff.correlate(mae) << "," <<
               lstm.correlate(mae) << "," << ugrnn.correlate(mae) << "," << delta.correlate(mae) << "," <<
               mgu.correlate(mae) << "," << gru.correlate(mae) << endl;
               */
    ...
    cout << "\\caption{\\label{table:consolidated_rankings} Hidden node count correlations for EXAMM runs evolving all "
            "memory neuron types.}"
         << endl;
    ...
    for (int32_t i = 0; i < (int32_t) run_statistics.size(); i++) {
        if (run_statistics[i]->run_type.find("all") != string::npos) {
            cout << run_statistics[i]->output_name << " & " << run_statistics[i]->ff.min() << " & " << setprecision(1)
                 ...
                 << run_statistics[i]->delta.min() << " & " << setprecision(1) << run_statistics[i]->delta.avg()
                 << " & " << run_statistics[i]->delta.max() << "&" << setprecision(2)
                 << run_statistics[i]->delta.correlate(run_statistics[i]->mse) << " & " << run_statistics[i]->mgu.min()
                 << " & " << setprecision(1) << run_statistics[i]->mgu.avg() << " & " << run_statistics[i]->mgu.max()
                 << "&" << setprecision(2) << run_statistics[i]->mgu.correlate(run_statistics[i]->mse) << " & "
                 << run_statistics[i]->gru.min() << " & " << setprecision(1) << run_statistics[i]->gru.avg() << " & "
                 << run_statistics[i]->gru.max() << "&" << setprecision(2)
                 << run_statistics[i]->gru.correlate(run_statistics[i]->mse) << "\\\\" << endl;
        }
    }
    ...
    for (int32_t i = 0; i < (int32_t) output_types.size(); i++) {
        ...
        cout << "\\hline" << endl;
        cout << "\\multicolumn{2}{|c||}{{\\bf Best Case}} & \\multicolumn{2}{|c||}{{\\bf Avg. Case}} & "
                "\\multicolumn{2}{|c|}{{\\bf Worst Case}} \\\\"
             << endl;
        cout << "\\hline" << endl;
        ...
        cout << "\\hline" << endl;
        cout << "\\end{tabular}" << endl;
        cout << "\\caption{\\label{table:consolidated_rankings} Rankings for all the EXAMM run types predicting "
             << output_types[i] << ".}" << endl;
        cout << "\\end{scriptsize}" << endl;
        cout << "\\end{table}" << endl;
    }
