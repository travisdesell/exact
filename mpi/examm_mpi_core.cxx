...
void send_msg_to_async(int target, unique_ptr<Msg> msg, int tag = WORK_TAG) {
  auto thread = std::thread(send_msg_to, target, move(msg), tag);
  thread.detach();
}
    ...
    case Msg::WORK: {
      ...
      if (genome == nullptr) {
        Log::fatal(
            ...
            "examm_mpi_core::generate_and_send_work\n");
        exit(1);
      }
...
void write_time_log(string path, vector<long> ls) {
  ...
  for (size_t i = 0; i < ls.size(); i++) { (*log_file) << ls[i] << endl; }
...
void master(int max_rank, GenomeOperators genome_operators) {
  Log::set_id("master");
  // the "main" id will have already been set by the main function so we do not
  // need to re-set it here
  ...
  for (int worker_rank = 1; worker_rank < max_rank; worker_rank++) {
    unique_ptr<Msg> init_work = examm->get_initialize_msg();
    send_msg_to(worker_rank, move(init_work), INITIALIZE_TAG);
  }
...
#define diff_as_nanos(start, end) std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count()
  ...
  while (terminates_sent < max_rank - 1) {
    ...
    Log::debug("probing...\n");
    MPI_Status status;
    // chrono::time_point<chrono::system_clock> start_probe =
    // chrono::system_clock::now();
    MPI_Probe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
    // chrono::time_point<chrono::system_clock> end_probe =
    // chrono::system_clock::now();
    ...
    if (tag == WORK_REQUEST_TAG) {
      ...
      // chrono::time_point<chrono::system_clock> start_gen =
      // chrono::system_clock::now();
#ifdef MASTER_PERFORMS_OPERATORS
...
#endif
      // chrono::time_point<chrono::system_clock> end_gen =
      // chrono::system_clock::now();
      ...
      // chrono::time_point<chrono::system_clock> start_recv_result =
      // chrono::system_clock::now();
      unique_ptr<Msg> msg = receive_msg_from(source, RESULT_TAG);
      // chrono::time_point<chrono::system_clock> end_recv_result =
      // chrono::system_clock::now();

      // long recv_result_nanos = diff_as_nanos(start_recv_result,
      // end_recv_result); recv_result_times.push_back(recv_result_nanos);
      ...
      // chrono::time_point<chrono::system_clock> start_insert =
      // chrono::system_clock::now();
      examm->insert_genome(move(genome));
      // chrono::time_point<chrono::system_clock> end_insert =
      // chrono::system_clock::now(); long insert_nanos =
      // diff_as_nanos(start_insert, end_insert);
  ...
  // write_time_log(output_directory + "/insert_genome_times.dat",
  // insert_genome_times); write_time_log(output_directory +
  // "/recv_result_times.dat", recv_result_times);
  // write_time_log(output_directory + "/probe_times.dat", probe_times);

  cout << "MASTER FINISHED >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n";
}

void worker(int n_workers, int rank, GenomeOperators genome_operators, string id = "") {
  Log::set_id("worker_" + to_string(rank) + "_" + id);
  Log::info("worker %d initialized\n", rank);

  unique_ptr<Msg> init_msg = receive_initialize();

  MPIInitMsg *init = static_cast<MPIInitMsg *>(init_msg.get());
  init->run(n_workers, rank);
...
void examm_mpi_main(int argc, char **argv) {
  ArgumentSet::set_argv(vector<string>(argv, argv + argc));
}
