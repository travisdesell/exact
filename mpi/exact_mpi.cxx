...
CNN_Genome *receive_genome_from(string name, int source) {
  ...
  MPI_Recv(length_message, 1, MPI_INT, source, GENOME_LENGTH_TAG,
           MPI_COMM_WORLD, &status);

  int length = length_message[0];

  cout << "[" << setw(10) << name << "] receiving genome of length: " << length
       << " from: " << source << endl;

  char *genome_str = new char[length + 1];

  cout << "[" << setw(10) << name << "] receiving genome from: " << source
       << endl;
  MPI_Recv(genome_str, length, MPI_CHAR, source, GENOME_TAG, MPI_COMM_WORLD,
           &status);
...
void send_genome_to(string name, int target, CNN_Genome *genome) {
  ...
  cout << "[" << setw(10) << name << "] sending genome of length: " << length
       << " to: " << target << endl;

  int length_message[1];
  length_message[0] = length;
  MPI_Send(length_message, 1, MPI_INT, target, GENOME_LENGTH_TAG,
           MPI_COMM_WORLD);

  cout << "[" << setw(10) << name << "] sending genome to: " << target << endl;
  MPI_Send(genome_str.c_str(), length, MPI_CHAR, target, GENOME_TAG,
           MPI_COMM_WORLD);
}
...
void master(const Images &training_images, const Images &validation_images,
            const Images &testing_images, int max_rank) {
  string name = "master";
  ...
  while (true) {
    ...
    cout << "[" << setw(10) << name
         << "] probe returned message from: " << source << " with tag: " << tag
         << endl;
    ...
    if (tag == WORK_REQUEST_TAG) {
      ...
      if (genome ==
          NULL) {  // search was completed if it returns NULL for an individual
        // send terminate message
        cout << "[" << setw(10) << name << "] terminating worker: " << source
             << endl;
        send_terminate_message(source);
        terminates_sent++;

        cout << "[" << setw(10) << name << "] sent: " << terminates_sent
             << " terminates of: " << (max_rank - 1) << endl;
        if (terminates_sent >= max_rank - 1) return;

      } else {
        ofstream outfile(exact->get_output_directory() + "/gen_" +
                         to_string(genome->get_generation_id()));
        genome->write(outfile);
        outfile.close();

        // send genome
        cout << "[" << setw(10) << name << "] sending genome to: " << source
             << endl;
      ...
      cout << "[" << setw(10) << name << "] received genome from: " << source
           << endl;
      ...
      cerr << "[" << setw(10) << name
           << "] ERROR: received message with unknown tag: " << tag << endl;
      MPI_Abort(MPI_COMM_WORLD, 1);
    }
  }
}

void worker(const Images &training_images, const Images &validation_images,
            const Images &testing_images, int rank) {
  ...
  while (true) {
    ...
    cout << "[" << setw(10) << name
         << "] probe received message with tag: " << tag << endl;
