...
__kernel void propagate_forward(const __global float *inputs, __constant float *weights, __global float *outputs, __local float *inputs_local) {
    int batch_number = get_group_id(0);
    int y = get_global_id(0);
    int x = get_global_id(1);

    int output_position = (y * output_x) + x;

    //printf("thread[%d][%d][%d] output position: %d\n", batch_number, y, x, output_position);
    y = y % output_y;

    int inputs_local_offset = ((y * output_x) + x) * 4;
    int input_offset = (batch_number * input_image_size) + inputs_local_offset;
    ...
    int width_difference = (input_x - output_x) - 1;
