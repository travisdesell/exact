...
RESULT_PATH=$EXAMM_PATH"/AAAI_EXAMM/multivariate"
DATASET="weather"

INPUT_PARAMETERS="p(mbar) T(degC) Tpot(K) Tdew(degC) rh(%) VPmax(mbar) VPact(mbar) VPdef(mbar) sh(g/kg) H2OC(mmol/mol) rho(g/m**3) wv(m/s) max.wv(m/s) wd(deg) rain(mm) raining(s) SWDR(W/m�) PAR(�mol/m�/s) max.PAR(�mol/m�/s) Tlog(degC) OT"
...
do
    ...
    do 
        ...
        --output_parameter_names $INPUT_PARAMETERS \
        --number_islands 10 \
        --island_size 10 \
        --max_genomes 10000 \
