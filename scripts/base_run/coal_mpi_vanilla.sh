...
for bp in $BPS; do
  echo "=== bp_iterations: $bp ==="
  for i in $(seq 1 $RUNS); do
    exp_name="../test_output/line_grid_search/coal_mpi_vanilla/bp_${bp}/run_${i}"
    mkdir -p "$exp_name"
    echo "Run ${i}/${RUNS} (bp_iterations=$bp): results will be saved to: $exp_name"
    echo "###-------------------###"
