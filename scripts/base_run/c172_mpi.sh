...
do
    ...
    exp_name="../test_output/c172_baseline/$i"
    mkdir -p $exp_name
    echo "Running base EXAMM code with c172 dataset, results will be saved to: "$exp_name
    echo "###-------------------###"

    mpirun -np 16 ./mpi/examm_mpi \
    --training_filenames ../datasets/2019_ngafid_transfer/c172_file_[1-9].csv --test_filenames ../datasets/2019_ngafid_transfer/c172_file_1[0-1].csv \
    --time_offset 1 \
