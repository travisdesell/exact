...
int main(int argc, char** argv) {
    ...
    for (int32_t max_recurrent_depth = 1; max_recurrent_depth <= 5; max_recurrent_depth++) {
        ...
        gradient_test("COS GP: 1 Input, 1 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs1, 1, 1, outputs1, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 1 Input, 1x1 Hidden, 1 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs1, 1, 2, outputs1, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 1 Input, 1x2 Hidden, 1 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs1, 2, 2, outputs1, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 1 Input, 2x2 Hidden, 1 Output", genome, inputs, outputs);
        delete genome;
        ...
        gradient_test("COS GP: 2 Input, 2 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs2, 2, 2, outputs2, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 2 Input, 2x2 Hidden, 2 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs2, 2, 3, outputs2, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 2 Input, 2x3 Hidden, 2 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs2, 3, 3, outputs2, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 2 Input, 3x3 Hidden, 2 Output", genome, inputs, outputs);
        delete genome;
        ...
        gradient_test("COS GP: Three Input, Three Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs3, 3, 3, outputs3, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 3 Input, 3x3 Hidden, 3 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs3, 3, 4, outputs3, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 3 Input, 3x4 Hidden, 3 Output", genome, inputs, outputs);
        delete genome;

        genome = create_cos_gp(inputs3, 4, 4, outputs3, max_recurrent_depth, weight_rules);
        gradient_test("COS GP: 3 Input, 4x4 Hidden, 3 Output", genome, inputs, outputs);
        delete genome;
    }
}
