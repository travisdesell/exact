...
EXAMM::EXAMM(int32_t _island_size, int32_t _number_islands, int32_t _max_genomes,
...
void EXAMM::print() {
    if (Log::at_level(Log::TRACE)) { speciation_strategy->print(); }
}
...
void EXAMM::update_log() {
    if (log_file != NULL) {
        ...
        if (best_genome == NULL) { best_genome = speciation_strategy->get_global_best_genome(); }
...
RNN_Genome *EXAMM::get_best_genome() { return speciation_strategy->get_best_genome(); }

RNN_Genome *EXAMM::get_worst_genome() { return speciation_strategy->get_worst_genome(); }
...
RNN_Genome *EXAMM::generate_genome() {
    ...
    function<RNN_Genome *(RNN_Genome *, RNN_Genome *)> crossover_function =
        [=](RNN_Genome *parent1, RNN_Genome *parent2) { return this->crossover(parent1, parent2); };
...
int32_t EXAMM::get_random_node_type() { return possible_node_types[rng_0_1(generator) * possible_node_types.size()]; }
...
void EXAMM::attempt_node_insert(vector<RNN_Node_Interface *> &child_nodes, const RNN_Node_Interface *node,
...
void EXAMM::attempt_edge_insert(vector<RNN_Edge *> &child_edges, vector<RNN_Node_Interface *> &child_nodes,
...
void EXAMM::attempt_recurrent_edge_insert(vector<RNN_Recurrent_Edge *> &child_recurrent_edges,
...
RNN_Genome *EXAMM::crossover(RNN_Genome *p1, RNN_Genome *p2) {
    ...
    while (p1_position < (int32_t) p1_recurrent_edges.size() && p2_position < (int32_t) p2_recurrent_edges.size()) {
        ...
        if (p1_innovation == p2_innovation) {
            // do weight crossover
            attempt_recurrent_edge_insert(child_recurrent_edges, child_nodes, p1_recurrent_edge, p2_recurrent_edge,
                                          true);
