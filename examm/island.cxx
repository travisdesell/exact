...
int32_t Island::size() { return (int32_t) genomes.size(); }
...
bool Island::is_initializing() { return status == Island::INITIALIZING; }

bool Island::is_repopulating() { return status == Island::REPOPULATING; }
...
int32_t Island::insert_genome(RNN_Genome *genome) {
    ...
    if (is_full() && new_fitness > get_worst_fitness()) {
        Log::debug("ignoring genome, fitness: %lf > worst for island[%d] fitness: %lf\n", new_fitness, id,
    ...
    if (structure_map.count(structural_hash) > 0) {
        ...
        Log::debug("potential duplicate for hash '%s', had %d potential matches.\n", structural_hash.c_str(),
    ...
    if (best.size() != 0) { copy->set_weights(best); }
    ...
    if ((int32_t) genomes.size() > max_size) {
        ...
        for (auto potential_match = potential_matches.begin(); potential_match != potential_matches.end();) {
            ...
            Log::debug("checking to remove worst from structure_map - &worst: %p, &(*potential_match): %p\n", worst,
                       (*potential_match));
        ...
        if (!found) {
            Log::debug("could not erase from structure_map[%s], genome not found! This should never happen.\n",
                       structural_hash.c_str());
            exit(1);
        }
...
void Island::erase_island() {
    ...
    for (int32_t i = 0; i < (int32_t) genomes.size(); i++) { delete genomes[i]; }
    genomes.clear();
    erased = true;
    erase_again = 5;
...
int32_t Island::get_status() { return status; }
...
void Island::fill_with_mutated_genomes(RNN_Genome *seed_genome, int32_t num_mutations, bool tl_epigenetic_weights,
