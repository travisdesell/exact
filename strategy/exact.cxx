...
CNN_Genome* EXACT::generate_individual() {
    ...
    if (genomes.size() == 0) {
        ...
        for (int32_t i = 0; i < number_classes; i++) {
            CNN_Edge *edge = new CNN_Edge(input_node, all_nodes.at(i + 1) /*ith softmax node*/, true, edge_innovation_count);
        ...
        if (r < edge_add) {
            ...
            if (edge_exists && !edge_exists_in_child) {
                ...
                if (!edge_copy->set_nodes(child->get_nodes())) {
                    ...
                    cout << "\t\tedge_copy->filter_x: " << edge_copy->get_filter_x() << endl;
                    cout << "\t\tedge_copy->filter_y: " << edge_copy->get_filter_y() << endl;
