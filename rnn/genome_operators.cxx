...
RNN_Genome *GenomeOperators::crossover(RNN_Genome *more_fit, RNN_Genome *less_fit) {
    ...
    while (more_fit_position < (int32_t)more_fit_edges.size() && less_fit_position < (int32_t)less_fit_edges.size()) {
        ...
        if (more_fit_innovation == less_fit_innovation) {
            ...
            if (less_fit_edge->is_reachable() && set_enabled)
                set_enabled = true;
            else
                set_enabled = false;
    ...
    while (more_fit_position < (int32_t)more_fit_recurrent_edges.size() &&
        ...
        if (more_fit_innovation == less_fit_innovation) {
            ...
            attempt_recurrent_edge_insert(child_recurrent_edges, child_nodes, more_fit_recurrent_edge, NULL,
                                          set_enabled);

            more_fit_position++;
        } else {
            bool set_enabled = rng_0_1(generator) < less_fit_crossover_rate;
            if (less_fit_recurrent_edge->is_reachable() && set_enabled)
                set_enabled = true;
            else
                set_enabled = false;

            attempt_recurrent_edge_insert(child_recurrent_edges, child_nodes, less_fit_recurrent_edge, NULL,
                                          set_enabled);
