...
RNN_Genome *GenomeOperators::crossover(RNN_Genome *more_fit,
  ...
  for (uint32_t i = 0; i < more_fit->nodes.size(); i++) {
    Log::debug(
        "more_fit node[%d], in: %d, depth: %lf, layer_type: %d, node_type: "
        "%d, "
  ...
  for (uint32_t i = 0; i < less_fit->nodes.size(); i++) {
    Log::debug(
        "less_fit node[%d], in: %d, depth: %lf, layer_type: %d, node_type: "
        "%d, "
        "reachable: %d, enabled: %d\n",
...
RNN_Genome *GenomeOperators::ncrossover(vector<const RNN_Genome *> &parents) {
  ...
  for (auto i = 0; i < parents.size(); i++) {
    ...
    for (auto j = 0; j < p->nodes.size(); j++) {
      ...
      if (node->enabled) {
        ...
        node_weights[node->innovation_number].push_back(
            {(int32_t)i, move(weights)});
        if (!node_enabled[node->innovation_number] &&
            rng_0_1(generator) < probability)
  ...
  // get_centroid_mass
  auto weight_crossover_1d =
      [this, get_centroid_mass](vector<pair<int32_t, double> > &weights) {
        ...
        double weight =
            crossover_weight * (centroid - weights.back().second) + centroid;
        return weight;
      };

  auto weight_crossover_2d =
      [this, get_centroid_mass](
          vector<pair<int32_t, vector<double> > > &weights,
        ...
        for (int i = 0; i < n_weights; i++) {
          new_weights.push_back(crossover_weight *
                                    (centroid[i] - worst_weights[i]) +
                                centroid[i]);
        }
      };
  ...
  Log::info("Child with %d nodes, %d edges, %d redges\n", child->nodes.size(),
            child->edges.size(), child->recurrent_edges.size());
