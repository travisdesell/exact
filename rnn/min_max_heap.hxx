...
class min_max_heap {
 ...
 public:
  ...
  const T &find_min() const { return heap[find_min_index()]; }

  /**
   * Returns and removes the maximum item in this min-max heap. If the heap is
  ...
  /**
   * Returns and removes the minimum item in this min-max heap. If the heap is
  ...
  /**
   * returns the const_iterator to the beginning of the underlying vector. There
   *is no non-const iterator because the ordering of the data structure could
   *potentially be changed.
   **/
  const_iterator cbegin() { return heap.begin(); }

  /**
   * returns the const_iterator pointing past the end of the underlying vector
   * (i.e. this is an invalid reference). There is no non-const end iterator
   *because the ordering of the data structure must be preserved.
   **/
  const_iterator cend() { return heap.end(); }

  /**
   * Implementation of the indexing operator to allow direct access.
   **/
  T &operator[](size_t index) { return heap[index]; }
   ...
   **/
  void reserve(size_t n) { heap.reserve(n); }
