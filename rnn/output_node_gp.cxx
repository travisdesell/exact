...
#include "output_node_gp.hxx"

OUTPUT_Node_GP::OUTPUT_Node_GP(int32_t _innovation_number, int32_t _layer_type, double _depth, string _parameter_name)
    : RNN_Node(_innovation_number, _layer_type, _depth, OUTPUT_NODE_GP, _parameter_name) {
    Log::debug("created node: %d, layer type: %d, node type: OUTPUT_NODE_GP\n", innovation_number, layer_type);
}

OUTPUT_Node_GP::~OUTPUT_Node_GP() {
}

double OUTPUT_Node_GP::activation_function(double input) {
    return input;
}

double OUTPUT_Node_GP::derivative_function(double input) {
    return 1;
}

void OUTPUT_Node_GP::input_fired(int32_t time, double incoming_output) {
    inputs_fired[time]++;
...
void OUTPUT_Node_GP::try_update_deltas(int32_t time) {
...
RNN_Node_Interface* OUTPUT_Node_GP::copy() const {
    OUTPUT_Node_GP* n = new OUTPUT_Node_GP(innovation_number, layer_type, depth, "");
