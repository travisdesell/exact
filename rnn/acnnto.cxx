...
void ACNNTO::write_to_stream(ostream &bin_ostream, bool verbose) {
    ...
    for (auto const& c : colony){
        bin_ostream.write((char*)&c.first, sizeof(int32_t));
        for (int n=0; n<node_types.size(); n++){
            if (c.first!=-1){
                bin_ostream.write((char*)&c.second->type_pheromones[n], sizeof(double));
            }
            for ( int e=0; e<c.second->pheromone_lines->size(); e++){
                ...
                double  edge_phermone            = c.second->pheromone_lines->at(e)->get_edge_innovation_number();
                int32_t depth                    = c.second->pheromone_lines->at(e)->get_edge_innovation_number();
                int32_t input_innovation_number  = c.second->pheromone_lines->at(e)->get_edge_innovation_number();
                int32_t output_innovation_number = c.second->pheromone_lines->at(e)->get_edge_innovation_number();
                ...
                bin_ostream.write((char*)&input_innovation_number   , sizeof(int));
                bin_ostream.write((char*)&output_innovation_number  , sizeof(int));
            }
        }
    }
}
