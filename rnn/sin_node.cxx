...
SIN_Node::SIN_Node(int32_t _innovation_number, int32_t _layer_type, double _depth, int32_t _node_type)
...
void SIN_Node::initialize_lamarckian(
...
void SIN_Node::initialize_xavier(minstd_rand0& generator, uniform_real_distribution<double>& rng_1_1, double range) {
    bias = range * (rng_1_1(generator));
}

void SIN_Node::initialize_kaiming(minstd_rand0& generator, NormalDistribution& normal_distribution, double range) {
    bias = range * normal_distribution.random(generator, 0, 1);
}

void SIN_Node::initialize_uniform_random(minstd_rand0& generator, uniform_real_distribution<double>& rng) {
    bias = rng(generator);
}

void SIN_Node::input_fired(int32_t time, double incoming_output) {
    inputs_fired[time]++;
...
void SIN_Node::error_fired(int32_t time, double error) {
    outputs_fired[time]++;
...
void SIN_Node::output_fired(int32_t time, double delta) {
    outputs_fired[time]++;
...
void SIN_Node::reset(int32_t _series_length) {
...
void SIN_Node::get_gradients(vector<double>& gradients) {
    gradients.assign(1, d_bias);
}

int32_t SIN_Node::get_number_weights() const {
    return 1;
}

void SIN_Node::get_weights(vector<double>& parameters) const {
...
void SIN_Node::set_weights(const vector<double>& parameters) {
    // no weights to set in a basic RNN node, only a bias
    int32_t offset = 0;
    set_weights(offset, parameters);
}

void SIN_Node::get_weights(int32_t& offset, vector<double>& parameters) const {
    // no weights to set in a basic RNN node, only a bias
    parameters[offset++] = bias;
}

void SIN_Node::set_weights(int32_t& offset, const vector<double>& parameters) {
    // no weights to set in a basic RNN node, only a bias
...
RNN_Node_Interface* SIN_Node::copy() const {
    ...
    if (layer_type == HIDDEN_LAYER) {
        n = new SIN_Node(innovation_number, layer_type, depth, node_type);
    } else {
        n = new SIN_Node(innovation_number, layer_type, depth, node_type, parameter_name);
    }

    // copy SIN_Node values
    n->bias = bias;
    n->d_bias = d_bias;
...
void SIN_Node::write_to_stream(ostream& out) {
