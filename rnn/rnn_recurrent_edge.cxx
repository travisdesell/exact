...
void RNN_Recurrent_Edge::first_propagate_forward() {
    for (uint32_t i = 0; i < recurrent_depth; i++) {
        cout << "FIRST propagating forward on recurrent edge " << innovation_number << " from node " << input_innovation_number << " to node " << output_innovation_number << " with recurrent depth: " << recurrent_depth << endl;
