...
WorkMsg::WorkMsg(shared_ptr<const RNN_Genome> g, uint32_t n_mutations)
    : args(mu_args{.g = move(g), .n_mutations = n_mutations}),
      work_type(mutation),
      is_shared(true) {}
...
WorkMsg::WorkMsg(shared_ptr<const RNN_Genome> g)
    : work_type(mutation),
      args(mu_args{.g = move(g), .n_mutations = 0}),
      is_shared(true) {}
  ...
  switch (work_type) {
    case crossover: {
      ...
      for (uint32_t i = 0; i < n; i++)
        parents.push_back(make_unique<RNN_Genome>(bin_istream));
      args = co_args{.parents = move(parents)};
  ...
  if (work_type == crossover) {
    ...
    if (cargs.parents.index() == shared) {
      ...
      for (int i = 0; i < parents.size(); i++)
        _work_msg_get_genome_genomes.push_back(parents[i].get());
    } else {
    ...
    RNN_Genome *parent = is_shared ? get<shared>(margs.g)->copy()
                                   : get<unique>(margs.g).release();
