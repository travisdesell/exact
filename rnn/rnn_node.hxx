...
class RNN_Node : public RNN_Node_Interface {
 ...
 public:
  ...
  RNN_Node(inon_type<RNN_Node_Interface> _inon, int _layer_type, double _depth, int _node_type);

  // constructor for input and output nodes
  RNN_Node(inon_type<RNN_Node_Interface> _inon, int _layer_type, double _depth, int _node_type, string _parameter_name);
  ~RNN_Node();
