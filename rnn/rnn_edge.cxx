...
RNN_Edge::RNN_Edge(int _innovation_number, int _input_innovation_number, int _output_innovation_number,
    ...
    for (int i = 0; i < nodes.size(); i++) {
        if (nodes[i]->innovation_number == _input_innovation_number) {
            if (input_node != NULL) {
                Log::fatal(
                    ...
                    "same input_innovation_number -- this should never happen.\n");
                exit(1);
            }
        ...
        if (nodes[i]->innovation_number == _output_innovation_number) {
            if (output_node != NULL) {
                Log::fatal(
                    "ERROR in copying RNN_Edge, list of nodes has multiple nodes with "
                    "same output_innovation_number -- this should never happen.\n");
                exit(1);
            }
...
RNN_Edge *RNN_Edge::copy(unordered_map<int32_t, RNN_Node_Interface *> new_nodes) {
...
void RNN_Edge::propagate_forward(int time) {
    ...
    // Log::trace("propagating forward at time %d from %d to %d, value: %lf,
    // input: %lf, weight: %lf\n", time, input_node->innovation_number,
    // output_node->innovation_number, output, input_node->output_values[time],
    // weight);
...
void RNN_Edge::propagate_backward(int time) {
    ...
    // Log::trace("propgating backward on edge %d at time %d from node %d to node
    // %d\n", innovation_number, time, output_innovation_number,
    // input_innovation_number);
...
void RNN_Edge::reset(uint32_t series_length) {
    d_weight = 0.0;
...
int32_t RNN_Edge::get_input_innovation_number() const { return input_innovation_number; }

int32_t RNN_Edge::get_output_innovation_number() const { return output_innovation_number; }
