...
Island::Island(int32_t _id, int32_t _max_size)
    : id(_id), max_size(_max_size), status(IslandStatus::INITIALIZING),
      erase_again(0), erased(false) {}

Island::Island(int32_t _id, vector<RNN_Genome *> _genomes)
    : id(_id), max_size(_genomes.size()), genomes(_genomes),
      status(IslandStatus::FILLED), erase_again(0), erased(false) {}
...
void Island::copy_random_genome(uniform_real_distribution<double> &rng_0_1,
                                minstd_rand0 &generator, RNN_Genome **genome) {
...
int32_t Island::insert_genome(RNN_Genome *genome) {
  ...
  if (erased_generation_id >= 0 &&
      genome->get_generation_id() <= erased_generation_id) {
  ...
  if (structure_map.count(structural_hash) > 0) {
    vector<RNN_Genome *> &potential_matches =
        structure_map.find(structural_hash)->second;

    Log::info("potential duplicate for hash '%s', had %d potential matches.\n",
              structural_hash.c_str(), potential_matches.size());

    for (auto potential_match = potential_matches.begin();
         potential_match != potential_matches.end();) {
      ...
      if ((*potential_match)->equals(genome)) {
        if ((*potential_match)->get_fitness() > new_fitness) {
          Log::info("REPLACING DUPLICATE GENOME, fitness of genome in search: "
                    "%s, new fitness: %s\n",
          ...
          // the duplicate it from the genomes as well from the potential matches
          // vector

          auto duplicate_genome_iterator =
              lower_bound(genomes.begin(), genomes.end(), *potential_match,
                          sort_genomes_by_fitness());
          bool found = false;
          for (; duplicate_genome_iterator != genomes.end();
          ...
          if (!found) {
            Log::fatal("ERROR: could not find duplicate genome even though its "
                       "structural hash was in the island, this should never "
                       "happen!\n");
            exit(1);
          }

          Log::info("potential_match->get_fitness(): %lf, "
                    "duplicate_genome_iterator->get_fitness(): %lf, "
                    "new_fitness: %lf\n",
                    (*potential_match)->get_fitness(),
                    (*duplicate_genome_iterator)->get_fitness(), new_fitness);

          int32_t duplicate_genome_index =
              duplicate_genome_iterator - genomes.begin();
          ...
          if (potential_matches.size() == 0) {
            Log::info("deleting the potential_matches vector for hash '%s' "
          ...
          Log::info("island already contains a duplicate genome with a better "
  ...
  auto index_iterator = upper_bound(genomes.begin(), genomes.end(), copy,
                                    sort_genomes_by_fitness());
  ...
  if (insert_index >= max_size) {
    ...
    Log::info(
        "not inserting genome because it is worse than the worst fitness\n");
    delete copy;
    do_population_check(__LINE__, initial_size);
    return -1;
  }
  ...
  if (genomes.size() > max_size) {
    ...
    for (auto potential_match = potential_matches.begin();
      ...
      Log::info("checking to remove worst from structure_map - &worst: %p, "
                "&(*potential_match): %p\n",
    ...
    if (!found) {
      Log::info("could not erase from structure_map[%s], genome not found! "
...
void Island::print(string indent) {
  if (Log::at_level(Log::INFO)) {
    ...
    Log::info("%s\t%s\n", indent.c_str(),
              RNN_Genome::print_statistics_header().c_str());

    for (uint32_t i = 0; i < genomes.size(); i++) {
      Log::info("%s\t%s\n", indent.c_str(),
                genomes[i]->print_statistics().c_str());
    }
  }
}
