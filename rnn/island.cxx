...
void Island::get_two_random_genomes(uniform_real_distribution<double> &rng_0_1,
...
void Island::get_n_random_genomes(
    uniform_real_distribution<double> &rng_0_1, minstd_rand0 &generator,
    int32_t n, vector<shared_ptr<const RNN_Genome>> &parents) {
  if (n > genomes.size()) {
...
// their pointer
pair<int32_t, const RNN_Genome *> Island::insert_genome(
  ...
  if (structure_map.count(structural_hash) > 0) {
    vector<shared_ptr<const RNN_Genome>> &potential_matches =
  ...
  auto index_iterator = upper_bound(genomes.begin(), genomes.end(), genome,
                                    sort_genomes_by_fitness());
