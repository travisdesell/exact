...
bool EXAMM::insert_genome(RNN_Genome* genome) {
    ...
    if(speciation_strategy->get_inserted_genomes() > fala_threshold){
        ...
        for (int32_t i = 0; i < rec_edges.size(); i++) {
            if (rec_edges[i]->is_reachable()) {
                int depth = rec_edges[i]->get_recurrent_depth() - 1;
                //Log::info("REdge depth: %d, index: %d\n", depth + 1, depth);
                if(insert_position >= 0){
                    recurrent_depth_reinforcement_signal[depth] += reinf;
                    num_rec_edges += 1.0;
                } else if (recurrent_depth_rates[depth] > 1.2/total_depths) {
                    recurrent_depth_reinforcement_signal[depth] -= 0.5;
                    num_rec_edges += 1.0;
                }
            }
        }
    ...
    if(speciation_strategy->get_inserted_genomes() > fala_threshold){
        ...
        double min_rec_depth_rate = 0.8/total_depths;
        if(num_rec_edges > 0){
            //Recurrent depth FALA
            for(int i = 0; i < total_depths; i++){
                recurrent_depth_reinforcement_signal[i] = recurrent_depth_reinforcement_signal[i]*fala_lr/num_rec_edges;
                recurrent_depth_rates[i] += recurrent_depth_reinforcement_signal[i];
                norm_factor += recurrent_depth_reinforcement_signal[i];
                if(recurrent_depth_rates[i] < min_rec_depth_rate){
                    norm_factor -= recurrent_depth_rates[i];
                    normalize_depth[i] = false;
                    recurrent_depth_rates[i] = min_rec_depth_rate;
