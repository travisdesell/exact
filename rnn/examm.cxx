...
EXAMM::EXAMM(int32_t population_size, int32_t number_islands, int32_t max_genomes, int32_t _max_time_minutes,
             int32_t extinction_event_generation_number, int32_t islands_to_exterminate, string island_ranking_method,
             string repopulation_method, int32_t repopulation_mutations, bool repeat_extinction,
             int32_t epochs_acc_freq, string speciation_method, double species_threshold, double fitness_threshold,
             double neat_c1, double neat_c2, double neat_c3, WeightType weight_initialize,
             WeightType weight_inheritance, WeightType mutated_component_weight, string output_directory,
             GenomeOperators _genome_operators, DatasetMeta dataset_meta, TrainingParameters training_parameters,
  ...
  if (seed_genome_was_minimal) {
    seed = unique_ptr<RNN_Genome>(create_ff(dataset_meta.input_parameter_names, 0, 0,
  ...
  if (speciation_method.compare("island") == 0 || speciation_method.compare("") == 0) {
    ...
    if (start_filled) { modify = apply_stir_mutations; }

    speciation_strategy =
        new IslandSpeciationStrategy(number_islands, population_size, seed_genome, island_ranking_method,
                                     repopulation_method, extinction_event_generation_number, repopulation_mutations,
                                     islands_to_exterminate, seed_genome_was_minimal, modify, genome_operators);
  ...
  if (output_directory != "") {
    ...
    (*log_file) << "Inserted Genomes, Total BP Epochs, Time, Best Val. MAE, Best Val. "
...
void EXAMM::print() {
  if (Log::at_level(Log::INFO)) { speciation_strategy->print(); }
}
...
void EXAMM::update_log() {
  ...
  (*log_file) << evaluated_genomes++ << "," << total_bp_epochs << "," << milliseconds << ","
              ...
              << speciation_strategy->get_strategy_information_values() << "," << best_genome->get_fitness() << ","
              << endl;
...
double EXAMM::get_best_fitness() { return speciation_strategy->get_best_fitness(); }

double EXAMM::get_worst_fitness() { return speciation_strategy->get_worst_fitness(); }

shared_ptr<const RNN_Genome> &EXAMM::get_best_genome() { return speciation_strategy->get_best_genome(); }

shared_ptr<const RNN_Genome> &EXAMM::get_worst_genome() { return speciation_strategy->get_worst_genome(); }
...
bool EXAMM::insert_genome(unique_ptr<RNN_Genome> unique_genome) {
  ...
  if (unique_genome->best_parameters.size() == unique_genome->initial_parameters.size())
    unique_genome->set_weights(unique_genome->best_parameters);

  Log::info("Inserting into speciation strategy\n");
  auto [insert_position, genome] = speciation_strategy->insert_genome(move(unique_genome));

  if (genome != nullptr) {
    const map<string, int> &generated_by_map = genome->get_generated_by_map();
    Log::info("Updating generated by map\n");
    for (auto it = generated_by_map.cbegin(); it != generated_by_map.cend(); it++) {
      if (generated_counts.count(it->first) > 0) {
        ...
        if (it->first != "initial") Log::error("unrecognized generated_by string '%s'\n", it->first.c_str());
      }
    }
    ...
    if (insert_position == 0) {
      ...
      genome->write_graphviz(output_directory + "/best.gv");
      genome->write_to_file(output_directory + "/best.bin");
    }
  }
...
unique_ptr<Msg> EXAMM::get_initialize_msg() {
  node_inon max_node = seed_genome->get_max_node_inon();
  edge_inon max_edge = seed_genome->get_max_edge_inon();

  MPIInitMsg *msg = new MPIInitMsg(node_inon(max_node.inon + 1), edge_inon(max_edge.inon + 1));

  return unique_ptr<Msg>((Msg *) msg);
}
