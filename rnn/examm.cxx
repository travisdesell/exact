...
EXAMM::EXAMM(
    ...
    fala_threshold = std::max(max_genomes/10, number_islands*population_size*12);
    //Minimum, starting, and degredation threshold values for each action probability
    mins = {0.06, 0.06, 0.06, 0.06, 0.06, 0, 0.06, 0.06, 0.06, 0.06, 0.06, 0.18, 0.09};
    start_rates = {0.07, 0.07, 0.07, 0.07, 0.07, 0, 0.07, 0.07, 0.07, 0.07, 0.07, 0.2, 0.1};
    degrade_thres = {0.09, 0.09, 0.09, 0.09, 0.09, 0, 0.09, 0.09, 0.09, 0.09, 0.09, 0.23, 0.12};
    ...
    if (node_ops) {
        ...
        rates[ENABLE_NODE_RATE_I] = start_rates[ADD_NODE_RATE_I];
        rates[DISABLE_NODE_RATE_I] = start_rates[ADD_NODE_RATE_I];
        rates[SPLIT_NODE_RATE_I] = start_rates[ADD_NODE_RATE_I];
        rates[MERGE_NODE_RATE_I] = start_rates[ADD_NODE_RATE_I];
    } else {
...
bool EXAMM::insert_genome(RNN_Genome* genome) {
        ...
        if (generated_counts.count(generated_by) > 0) {
            ...
            if (insert_position >= 0) {
                ...
                if(rates[index] > degrade_thres[index]){
                    reinforcement_signal[index] -= 0.4*rates[index]/degrade_thres[index];
                    num_mutations += 1.0;
                }
            }
    ...
    if(speciation_strategy->get_inserted_genomes() > fala_threshold and num_mutations > 0){
