...
EXAMM::EXAMM(
    ...
    recurrent_depth_reinforcement_signal.resize(NUM_RATES);
    //Set the FALA learning rate
    fala_lr = 0.001;
    //Calculate the threshold to start FALA
    fala_threshold = std::max(max_genomes/10, number_islands*population_size*8);
    //Minimum, starting, and degredation threshold values for each action probability
    mins = {0.055, 0.055, 0.055, 0.055, 0.055, 0, 0.055, 0.055, 0.055, 0.055, 0.055, 0.17, 0.09};
    start_rates = {0.07, 0.07, 0.07, 0.07, 0.07, 0, 0.07, 0.07, 0.07, 0.07, 0.07, 0.2, 0.1};
    degrade_thres = {0.075, 0.075, 0.075, 0.075, 0.075, 0, 0.075, 0.075, 0.075, 0.075, 0.075, 0.21, 0.105};
    ...
    for(int i = 0; i < total_depths; i++){
        ...
        Log::info("recurrent_depth_rate[%d] = %f\n", (i + 1), recurrent_depth_rates[i]);
    }
...
bool EXAMM::insert_genome(RNN_Genome* genome) {
    ...
    for (auto it = generated_by_map->begin(); it != generated_by_map->end(); it++) {
        ...
        if(generated_by.find("add_node") == 0){
            ...
            for (int32_t i = 0; i < recurrent_edges.size(); i++) {
                if (recurrent_edges[i]->is_reachable()) {
                    ...
                    count++;*/
                    Log:info("Depth of recurrent edge %d: %d\n", i, recurrent_edges[i]->get_recurrent_depth());
                }
            }
        }
        if (generated_counts.count(generated_by) > 0) {
            ...
            /*if(generated_by_map.find("add_recurrent_edge") == 0){
                Log::info("Recurrent edge depth: %d\n", gen_recurrent_depth);
            }*/
            if (insert_position >= 0) {
                ...
                if(speciation_strategy->get_inserted_genomes() > fala_threshold){
                    reinforcement_signal[index] += (population_size - insert_position + 1.0)/population_size;
                    reinforcement_signal[index] += 1.0/(genome->get_fitness()/speciation_strategy->get_best_genome()->get_fitness());
                    num_mutations += 1.0;
                    /*if(generated_by_map.find("add_recurrent_edge") == 0){
                        recurrent_depth_reinf += 1.0/(genome->get_fitness()/speciation_strategy->get_best_genome()->get_fitness());
                    }*/
                }
                
            } else {
                if(rates[index] > degrade_thres[index]){
                    reinforcement_signal[index] -= 0.5*rates[index]/degrade_thres[index];
                    num_mutations += 1.0;
                }
                /*if(generated_by_map.find("add_recurrent_edge") == 0 && recurrent_depth_rates[gen_recurrent_depth - 1] > 1.0/total_depths){
                        recurrent_depth_reinf -= 0.5;
                }*/
            }
    ...
    if(speciation_strategy->get_inserted_genomes() > fala_threshold){
        ...
        //Recurrent depth FALA
        /*Log::info("Recurrent depth reinforcement: %f\n", recurrent_depth_reinf);
        double rec_norm_factor = 1.0 + recurrent_depth_reinf*recurrent_depth_lr;
        recurrent_depth_rates[gen_recurrent_depth - 1] += recurrent_depth_reinf*recurrent_depth_lr;
        for(int i = 0; i < total_depths; i++){
            recurrent_depth_rates[i] /= rec_norm_factor;
