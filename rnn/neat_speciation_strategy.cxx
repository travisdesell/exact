...
NeatSpeciationStrategy::NeatSpeciationStrategy(
    ...
    double _species_threshold, double _fitness_threshold, double _neat_c1,
    double _neat_c2, double _neat_c3, minstd_rand0 &_generator)
    : generation_species(0), species_count(0),
      population_not_improving_count(0), species_threshold(_species_threshold),
      fitness_threshold(_fitness_threshold), neat_c1(_neat_c1),
      neat_c2(_neat_c2), neat_c3(_neat_c3), mutation_rate(_mutation_rate),
      intra_island_crossover_rate(_intra_island_crossover_rate),
      inter_island_crossover_rate(_inter_island_crossover_rate),
      generated_genomes(0), inserted_genomes(0), minimal_genome(_seed_genome),
...
int32_t NeatSpeciationStrategy::insert_genome(RNN_Genome *genome) {
  ...
  if (!inserted) {
    ...
    if (species_count != neat_species.size()) {
      Log::error("this should never happen, the species count is not the same "
  ...
  if (insert_position == 0) {
    ...
    population_not_improving_count++;
    return insert_position; // will be -1 if not inserted, or > 0 if not the
                            // global best
  }
}
...
WorkMsg *NeatSpeciationStrategy::generate_work(
  ...
  Log::info("generating new genome for species[%d], species_size: %d, "
            "mutation_rate: %lf, intra_island_crossover_rate: %lf, "
            "inter_island_crossover_rate: %lf\n",
            generation_species, currentSpecies->size(), mutation_rate,
  ...
  if (currentSpecies->size() <= 2) {
    ...
    if (currentSpecies->size() > 10)
      currentSpecies->fitness_sharing_remove(fitness_threshold,
                                             distance_function);
...
WorkMsg *NeatSpeciationStrategy::generate_work_for_species(
  ...
  if (r < mutation_rate) {
    ...
    RNN_Genome *parent2 =
        neat_species[other_island]->get_best_genome()->copy(); // new RNN GENOME
...
vector<uint32_t> NeatSpeciationStrategy::get_random_species_list() {
  ...
  for (uint32_t i = 0; i < neat_species.size(); i++)
...
void NeatSpeciationStrategy::rank_species() {
  ...
  for (uint32_t i = 0; i < neat_species.size() - 1; i++) {
    for (uint32_t j = 0; j < neat_species.size() - i - 1; j++) {
...
bool NeatSpeciationStrategy::check_population() {
  ...
  if (population_not_improving_count >= 3000) {
    Log::error("the population fitness has not been improved for 3000 genomes, "
               "start to erasing \n");
    rank_species();

    neat_species.erase(neat_species.begin(), neat_species.end() - 2);
    if (neat_species.size() != 2) {
      Log::error("It should never happen, the population has %d number of "
...
void NeatSpeciationStrategy::check_species() {
  ...
  while (it != neat_species.end()) {
    if (neat_species[it - neat_species.begin()]
      ...
      Log::error("Species at position %d hasn't been improving for 2250 "
                 "genomes, erasing it \n",
