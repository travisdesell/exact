...
pair<int32_t, const RNN_Genome *> NeatSpeciationStrategy::insert_genome(unique_ptr<RNN_Genome> genome) {
    ...
    if (insert_position == 0) {
        ...
        population_not_improving_count++;
        return pair(insert_position, g);  // will be -1 if not inserted, or > 0 if not the
                                 // global best
    }
}
...
unique_ptr<WorkMsg> NeatSpeciationStrategy::generate_work(uniform_real_distribution<double> &rng_0_1,
    ...
    if (current_species.size() < 2) {
        ...
        work = make_unique<WorkMsg>(genome, genome_operators.get_random_n_mutations());
    } else {
...
unique_ptr<WorkMsg> NeatSpeciationStrategy::generate_work_for_species(Species &species, uniform_real_distribution<double> &rng_0_1,
                                                                      minstd_rand0 &generator) {
    ...
    if (r < GenomeOperators::mutation_p) {
        ...
        return make_unique<WorkMsg>(genome, genome_operators.get_random_n_mutations());
    } else {
        r = rng_0_1(generator);
