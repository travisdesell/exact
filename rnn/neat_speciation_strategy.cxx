...
NeatSpeciationStrategy::NeatSpeciationStrategy(
    shared_ptr<const RNN_Genome> _seed_genome, double _species_threshold,
    double _fitness_threshold, double _neat_c1, double _neat_c2,
    double _neat_c3, minstd_rand0 &_generator, GenomeOperators genome_operators)
...
shared_ptr<const RNN_Genome> &NeatSpeciationStrategy::get_best_genome() {
...
pair<int32_t, const RNN_Genome *> NeatSpeciationStrategy::insert_genome(
  ...
  if (neat_species.size() == 1 && neat_species[0].size() == 0) {
    ...
    for (uint32_t i = 0; i < species_list.size(); i++) {
      ...
      const RNN_Genome *representative_genome =
          random_species.get_representative_genome();
...
unique_ptr<WorkMsg> NeatSpeciationStrategy::generate_work(
  ...
  function<double(const RNN_Genome *, const RNN_Genome *)> distance_function =
      [=](const RNN_Genome *g1, const RNN_Genome *g2) {
  ...
  Log::info(
      ...
      generation_species, current_species.size(), mutation_rate,
  ...
  if (current_species.size() < 2) {
    ...
    shared_ptr<const RNN_Genome> genome =
        current_species.get_random_genome(rng_0_1, generator);

    work =
        make_unique<WorkMsg>(genome, genome_operators.get_random_n_mutations());
  } else {
    // first eliminate genomes who have low fitness sharing in this species
    if (current_species.size() > 10)
      current_species.fitness_sharing_remove(fitness_threshold,
                                             distance_function);
...
unique_ptr<WorkMsg> NeatSpeciationStrategy::generate_work_for_species(
  ...
  if (r < GenomeOperators::mutation_p) {
    ...
    shared_ptr<const RNN_Genome> genome =
        species.get_random_genome(rng_0_1, generator);

    return make_unique<WorkMsg>(genome,
    ...
    if (r < GenomeOperators::intra_co_p || neat_species.size() == 1) {
      ...
      int32_t n_parents =
          min(species.size(), genome_operators.get_random_n_parents_intra());
      ...
      shared_ptr<const RNN_Genome> parent =
          species.get_random_genome(rng_0_1, generator);
      ...
      if (species.get_id() == neat_species[other_island].get_id())
        other_island++;

      Species &other = neat_species[other_island];
      int32_t n =
          min(other.size(), genome_operators.get_random_n_parents_inter() - 1);
...
shared_ptr<const RNN_Genome> &NeatSpeciationStrategy::get_global_best_genome() {
  return global_best_genome;
}
...
double NeatSpeciationStrategy::get_distance(const RNN_Genome *g1,
                                            const RNN_Genome *g2) {
  double distance;
  int E;
  int D;
  ...
  std::set_union(innovation1.begin(), innovation1.end(), innovation2.begin(),
                 innovation2.end(), std::inserter(setunion, setunion.begin()));
  std::set_intersection(innovation1.begin(), innovation1.end(),
                        innovation2.begin(), innovation2.end(),
...
// v1.max > v2.max
int NeatSpeciationStrategy::get_exceed_number(vector<int32_t> v1,
                                              vector<int32_t> v2) {
  int exceed = 0;
...
void NeatSpeciationStrategy::rank_species() {
  ...
  auto by = [&](Species &a, Species &b) -> bool {
    return a.get_best_fitness() > b.get_best_fitness();
  };
...
void NeatSpeciationStrategy::check_species() {
  ...
  while (it != neat_species.end()) {
    if (neat_species[it - neat_species.begin()]
            .get_species_not_improving_count() >= 2250) {
      Log::error(
