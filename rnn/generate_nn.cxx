...
RNN_Genome* create_lstm(const vector<string> &input_parameter_names, int number_hidden_layers, int number_hidden_nodes, const vector<string> &output_parameter_names, WeightType weight_initialize, Recurrent_Depth *node_rec_depth) {
    ...
    int edge_innovation_count = 0;
    int current_layer = 0;
    int node_recurrent_depth = node_rec_depth->get_recurrent_depth();
    ...
    for (int32_t i = 0; i < number_hidden_layers; i++) {
        for (int32_t j = 0; j < number_hidden_nodes; j++) {
            ...
            LSTM_Node *node = new LSTM_Node(++node_innovation_count, HIDDEN_LAYER, current_layer, node_recurrent_depth, node_rec_depth);
            rnn_nodes.push_back(node);
            layer_nodes[current_layer].push_back(node);
...
RNN_Genome* create_ugrnn(const vector<string> &input_parameter_names, int number_hidden_layers, int number_hidden_nodes, const vector<string> &output_parameter_names, WeightType weight_initialize, Recurrent_Depth *node_rec_depth) {
    Log::debug("creating UGRNN network with inputs: %d, hidden: %dx%d, outputs: %d\n", input_parameter_names.size(), number_hidden_layers, number_hidden_nodes, output_parameter_names.size());
    vector<RNN_Node_Interface*> rnn_nodes;
    vector< vector<RNN_Node_Interface*> > layer_nodes(2 + number_hidden_layers);
    vector<RNN_Edge*> rnn_edges;
    vector<RNN_Recurrent_Edge*> recurrent_edges;
    ...
    for (int32_t i = 0; i < number_hidden_layers; i++) {
        for (int32_t j = 0; j < number_hidden_nodes; j++) {
            UGRNN_Node *node = new UGRNN_Node(++node_innovation_count, HIDDEN_LAYER, current_layer, node_recurrent_depth, node_rec_depth);
            rnn_nodes.push_back(node);
            layer_nodes[current_layer].push_back(node);
...
RNN_Genome* create_gru(const vector<string> &input_parameter_names, int number_hidden_layers, int number_hidden_nodes, const vector<string> &output_parameter_names, WeightType weight_initialize, Recurrent_Depth *node_rec_depth) {
    Log::debug("creating GRU network with inputs: %d, hidden: %dx%d, outputs: %d\n", input_parameter_names.size(), number_hidden_layers, number_hidden_nodes, output_parameter_names.size());
    vector<RNN_Node_Interface*> rnn_nodes;
    vector< vector<RNN_Node_Interface*> > layer_nodes(2 + number_hidden_layers);
    vector<RNN_Edge*> rnn_edges;
    vector<RNN_Recurrent_Edge*> recurrent_edges;
    ...
    for (int32_t i = 0; i < number_hidden_layers; i++) {
        for (int32_t j = 0; j < number_hidden_nodes; j++) {
            GRU_Node *node = new GRU_Node(++node_innovation_count, HIDDEN_LAYER, current_layer, node_recurrent_depth, node_rec_depth);
            rnn_nodes.push_back(node);
            layer_nodes[current_layer].push_back(node);
...
RNN_Genome* create_mgu(const vector<string> &input_parameter_names, int number_hidden_layers, int number_hidden_nodes, const vector<string> &output_parameter_names, WeightType weight_initialize, Recurrent_Depth *node_rec_depth) {
    Log::debug("creating MGU network with inputs: %d, hidden: %dx%d, outputs: %d\n", input_parameter_names.size(), number_hidden_layers, number_hidden_nodes, output_parameter_names.size());
    vector<RNN_Node_Interface*> rnn_nodes;
    vector< vector<RNN_Node_Interface*> > layer_nodes(2 + number_hidden_layers);
    vector<RNN_Edge*> rnn_edges;
    vector<RNN_Recurrent_Edge*> recurrent_edges;
    ...
    for (int32_t i = 0; i < number_hidden_layers; i++) {
        for (int32_t j = 0; j < number_hidden_nodes; j++) {
            MGU_Node *node = new MGU_Node(++node_innovation_count, HIDDEN_LAYER, current_layer, node_recurrent_depth, node_rec_depth);
            rnn_nodes.push_back(node);
            layer_nodes[current_layer].push_back(node);
...
RNN_Genome* create_delta(const vector<string> &input_parameter_names, int number_hidden_layers, int number_hidden_nodes, const vector<string> &output_parameter_names, WeightType weight_initialize, Recurrent_Depth *node_rec_depth) {
    Log::debug("creating delta network with inputs: %d, hidden: %dx%d, outputs: %d\n", input_parameter_names.size(), number_hidden_layers, number_hidden_nodes, output_parameter_names.size());
    vector<RNN_Node_Interface*> rnn_nodes;
    vector< vector<RNN_Node_Interface*> > layer_nodes(2 + number_hidden_layers);
    vector<RNN_Edge*> rnn_edges;
    vector<RNN_Recurrent_Edge*> recurrent_edges;
    ...
    for (int32_t i = 0; i < number_hidden_layers; i++) {
        for (int32_t j = 0; j < number_hidden_nodes; j++) {
            Delta_Node *node = new Delta_Node(++node_innovation_count, HIDDEN_LAYER, current_layer, node_recurrent_depth, node_rec_depth);
            rnn_nodes.push_back(node);
            layer_nodes[current_layer].push_back(node);
