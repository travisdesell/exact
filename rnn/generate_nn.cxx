...
RNN_Genome *create_ff(const vector<string> &input_parameter_names, uint32_t number_hidden_layers,
                      uint32_t number_hidden_nodes, const vector<string> &output_parameter_names,
                      uint32_t max_recurrent_depth, TrainingParameters tp, WeightType weight_initialize,
  ...
  for (uint32_t i = 0; i < input_parameter_names.size(); i++) {
    RNN_Node *node = new RNN_Node(node_inon(), INPUT_LAYER, current_layer, SIMPLE_NODE, input_parameter_names[i]);
  ...
  for (uint32_t i = 0; i < number_hidden_layers; i++) {
    for (uint32_t j = 0; j < number_hidden_nodes; j++) {
      RNN_Node *node = new RNN_Node(node_inon(), HIDDEN_LAYER, current_layer, SIMPLE_NODE);
      rnn_nodes.push_back(node);
      layer_nodes[current_layer].push_back(node);

      for (uint32_t k = 0; k < layer_nodes[current_layer - 1].size(); k++) {
        rnn_edges.push_back(new RNN_Edge(edge_inon(), layer_nodes[current_layer - 1][k], node));

        for (uint32_t d = 1; d <= max_recurrent_depth; d++) {
          recurrent_edges.push_back(new RNN_Recurrent_Edge(edge_inon(), d, layer_nodes[current_layer - 1][k], node));
        }
      }
    }
    current_layer++;
  ...
  for (uint32_t i = 0; i < output_parameter_names.size(); i++) {
    RNN_Node *output_node =
        new RNN_Node(node_inon(), OUTPUT_LAYER, current_layer, SIMPLE_NODE, output_parameter_names[i]);
    rnn_nodes.push_back(output_node);

    for (uint32_t k = 0; k < layer_nodes[current_layer - 1].size(); k++) {
      rnn_edges.push_back(new RNN_Edge(edge_inon(), layer_nodes[current_layer - 1][k], output_node));

      for (uint32_t d = 1; d <= max_recurrent_depth; d++) {
        recurrent_edges.push_back(
            new RNN_Recurrent_Edge(edge_inon(), d, layer_nodes[current_layer - 1][k], output_node));
      }
    }
  }

  RNN_Genome *genome = new RNN_Genome(rnn_nodes, rnn_edges, recurrent_edges, tp, weight_initialize, weight_inheritance,
                                      mutated_component_weight);
  genome->set_parameter_names(input_parameter_names, output_parameter_names);
  return genome;
}

RNN_Genome *create_jordan(const vector<string> &input_parameter_names, uint32_t number_hidden_layers,
                          uint32_t number_hidden_nodes, const vector<string> &output_parameter_names,
                          uint32_t max_recurrent_depth, TrainingParameters tp, WeightType weight_initialize) {
  Log::debug(
  ...
  for (uint32_t i = 0; i < number_hidden_layers; i++) {
    for (uint32_t j = 0; j < number_hidden_nodes; j++) {
      RNN_Node *node = new RNN_Node(node_inon(), HIDDEN_LAYER, current_layer, JORDAN_NODE);
      rnn_nodes.push_back(node);
      layer_nodes[current_layer].push_back(node);
  ...
  for (uint32_t k = 0; k < output_layer.size(); k++) {
    for (uint32_t i = 0; i < number_hidden_layers; i++) {
      for (uint32_t j = 0; j < number_hidden_nodes; j++) {
        for (uint32_t d = 1; d <= max_recurrent_depth; d++) {
          recurrent_edges.push_back(new RNN_Recurrent_Edge(edge_inon(), d, output_layer[k], layer_nodes[i + 1][j]));
        }
      }
    }
  }
  ...
  RNN_Genome *genome =
      new RNN_Genome(rnn_nodes, rnn_edges, recurrent_edges, tp, weight_initialize, WeightType::NONE, WeightType::NONE);
  genome->set_parameter_names(input_parameter_names, output_parameter_names);
  return genome;
}

RNN_Genome *create_elman(const vector<string> &input_parameter_names, uint32_t number_hidden_layers,
                         uint32_t number_hidden_nodes, const vector<string> &output_parameter_names,
                         uint32_t max_recurrent_depth, TrainingParameters tp, WeightType weight_initialize) {
  Log::debug(
  ...
  for (uint32_t i = 0; i < number_hidden_layers; i++) {
    for (uint32_t j = 0; j < number_hidden_nodes; j++) {
      RNN_Node *node = new RNN_Node(node_inon(), HIDDEN_LAYER, current_layer, ELMAN_NODE);
      rnn_nodes.push_back(node);
      layer_nodes[current_layer].push_back(node);
    ...
    for (uint32_t j = 0; j < number_hidden_nodes; j++) {
      for (uint32_t k = 0; k < number_hidden_layers; k++) {
        for (uint32_t l = 0; l < number_hidden_nodes; l++) {
          for (uint32_t d = 1; d <= max_recurrent_depth; d++) {
            recurrent_edges.push_back(
                new RNN_Recurrent_Edge(edge_inon(), d, layer_nodes[i + 1][j], layer_nodes[k + 1][l]));
          }
        }
      }
    }
...
RNN_Genome *create_memory_cell_nn(const vector<string> &input_parameter_names, uint32_t number_hidden_layers,
                                  uint32_t number_hidden_nodes, const vector<string> &output_parameter_names,
                                  uint32_t max_recurrent_depth, TrainingParameters tp, WeightType weight_initialize) {
  ...
  for (uint32_t i = 0; i < number_hidden_layers; i++) {
    for (uint32_t j = 0; j < number_hidden_nodes; j++) {
      RNN_Node_Interface *node = new MemoryCell(node_inon(), HIDDEN_LAYER, current_layer);
      rnn_nodes.push_back(node);
      layer_nodes[current_layer].push_back(node);
