 ...
 *
 *
 * ./rnn_examples/evaluate_rnns_multi_offset --genome_filenames
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/boiler_best_bin_gv_files/nose_gas_temperature/time_offset_*.bin
 * --time_offsets 1 2 4 8 --testing_filenames
 * ~/Dropbox/1537\ MTI-RIT/boiler_hourly/database_l_minus_10.csv
 * --output_filename
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/nose_gas_temperature_predictons.csv
 *
 * ./rnn_examples/evaluate_rnns_multi_offset --genome_filenames
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/boiler_best_bin_gv_files/net_plant_heat_rate/time_offset_*.bin
 * --time_offsets 1 2 4 8 --testing_filenames
 * ~/Dropbox/1537\ MTI-RIT/boiler_hourly/database_l_minus_10.csv
 * --output_filename
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/net_plant_heat_rate_predictions.csv
 *
 * ./rnn_examples/evaluate_rnns_multi_offset --genome_filenames
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/plant_best_bin_gv_files/cyclone3/extra_plant_parameters/time_offset_*.bin
 * --time_offsets 1 15 30 60 120 240 480 --testing_filenames
 * ~/Dropbox/1537\ MTI-RIT/Field_Test_data/20190910_v0.2/cyclone3_file4.csv
 * --output_filename
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/flame_intensity_extra_3.csv
 *
 * ./rnn_examples/evaluate_rnns_multi_offset --genome_filenames
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/plant_best_bin_gv_files/cyclone3/plant_fuel_parameters/time_offset_*.bin
 * --time_offsets 1 15 30 60 120 240 480 --testing_filenames
 * ~/Dropbox/1537\ MTI-RIT/Field_Test_data/20190910_v0.2/cyclone3_file4.csv
 * --output_filename
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/flame_intensity_plant_fuel_3.csv
 *
 * ./rnn_examples/evaluate_rnns_multi_offset --genome_filenames
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/plant_best_bin_gv_files/cyclone3/plant_parameters/time_offset_*.bin
 * --time_offsets 1 15 30 60 120 240 480 --testing_filenames
 * ~/Dropbox/1537\ MTI-RIT/Field_Test_data/20190910_v0.2/cyclone3_file4.csv
 * --output_filename
 * ~/Dropbox/1537\ MTI-RIT/2019_degruyter_results/flame_intensity_plant_3.csv
 *
 * then you can use plot_multi_time_series.py to generate a chart of the time
 * series
 */
...
int main(int argc, char** argv) {
  ...
  get_argument_vector(arguments, "--testing_filenames", true,
                      testing_filenames);
  ...
  TimeSeriesSets* time_series_sets = TimeSeriesSets::generate_test(
      testing_filenames, genomes[0]->get_input_parameter_names(),
      genomes[0]->get_output_parameter_names());

  string normalize_type = genomes[0]->get_normalize_type();
  if (normalize_type.compare("min_max") == 0) {
    time_series_sets->normalize_min_max(genomes[0]->get_normalize_mins(),
                                        genomes[0]->get_normalize_maxs());
  } else if (normalize_type.compare("avg_std_dev") == 0) {
    time_series_sets->normalize_avg_std_dev(
        genomes[0]->get_normalize_avgs(), genomes[0]->get_normalize_std_devs(),
  ...
  Log::debug(
      "output_parameter_name: %s, full_series.size(): %d, "
      "full_series[0].size(): %d\n",
  ...
  for (int32_t i = 0; i < genomes.size(); i++) {
    time_series_sets = TimeSeriesSets::generate_test(
        testing_filenames, genomes[i]->get_input_parameter_names(),
        genomes[i]->get_output_parameter_names());
    Log::debug("got time series sets.\n");
    string normalize_type = genomes[i]->get_normalize_type();
    if (normalize_type.compare("min_max") == 0) {
      time_series_sets->normalize_min_max(genomes[i]->get_normalize_mins(),
                                          genomes[i]->get_normalize_maxs());
    } else if (normalize_type.compare("avg_std_dev") == 0) {
      time_series_sets->normalize_avg_std_dev(
          genomes[i]->get_normalize_avgs(),
    ...
    time_series_sets->export_test_series(time_offsets[i], testing_inputs,
                                         testing_outputs);

    vector<double> best_parameters = genomes[i]->get_best_parameters();
    Log::info("MSE: %lf\n", genomes[i]->get_mse(best_parameters, testing_inputs,
                                                testing_outputs));
    Log::info("MAE: %lf\n", genomes[i]->get_mae(best_parameters, testing_inputs,
                                                testing_outputs));

    vector<vector<double> > predictions = genomes[i]->get_predictions(
        best_parameters, testing_inputs, testing_outputs);

    Log::debug("predictions.size(): %d\n", predictions.size());

    if (predictions.size() != 1) {
      Log::fatal(
          "ERROR: had more than one testing file, currently only one "
          "supported.\n");
      exit(1);
    }
