...
inline WeightUpdateMethod get_enum_method_from_string(string input_string) {
    ...
    for (int i = 0; i < NUM_WEIGHT_UPDATE_TYPES; i++) {
        if (input_string.compare(WEIGHT_UPDATE_METHOD_STRING[i]) == 0) { method = static_cast<WeightUpdateMethod>(i); }
    }
    return method;
}
...
class WeightUpdate {
   ...
   public:
    ...
    void update_weights(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                        vector<double> &gradient, int32_t epoch);

    void vanilla_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                               vector<double> &gradient, int32_t epoch);
    void momentum_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                                vector<double> &gradient, int32_t epoch);
    void nesterov_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                                vector<double> &gradient, int32_t epoch);
    void adagrad_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                               vector<double> &gradient, int32_t epoch);
    void rmsprop_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                               vector<double> &gradient, int32_t epoch);
    void adam_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                            vector<double> &gradient, int32_t epoch);
    void adam_bias_weight_update(vector<double> &parameters, vector<double> &velocity, vector<double> &prev_velocity,
                                 vector<double> &gradient, int32_t epoch);
