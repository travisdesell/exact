...
void CNN_Genome::export_to_database(int _exact_id) {
    ...
    query << "'"
          ...
          << ", number_training_images = " << number_training_images << ", training_error = " << setprecision(15)
          << fixed << training_error << ", training_predictions = " << training_predictions
...
int CNN_Genome::get_operations_estimate() const {
    ...
    for (uint32_t i = 0; i < edges.size(); i++) {
        ...
        operations_estimate += edges[i]->get_filter_x() * edges[i]->get_filter_y() *
                               (4.0 * multiply_cost + 4.0 * add_cost + 2.0 * if_cost);
    }
...
bool CNN_Genome::sanity_check(int type) {
    ...
    for (uint32_t i = 0; i < edges.size(); i++) {
        if (edges[i]->get_filter_x() <= 0 || edges[i]->get_filter_x() > 100) {
            ...
            cerr << "edge in position " << i << " with innovation number: " << edges[i]->get_innovation_number()
                 << endl;
        ...
        */
        ...
        for (uint32_t i = 0; i < edges.size(); i++) {
            if (edges[i]->has_zero_best_weight()) {
                cerr << "ERROR before insert!" << endl;
                cerr << "ERROR: edge in position " << i << " with innovation number: " <<
        edges[i]->get_innovation_number() << endl; cerr << "sum of best weights was 0" << endl; cerr << "filter_x: " <<
        edges[i]->get_filter_x() << ", filter_y: " << edges[i]->get_filter_y() << endl; return false;
            }
        }
        */
        // cout << "passed checking zero best weights" << endl;
    ...
    for (uint32_t i = 0; i < nodes.size(); i++) {
        ...
        for (uint32_t j = 0; j < edges.size(); j++) {
            ...
            if (edges[j]->get_output_node()->get_innovation_number() == nodes[i]->get_innovation_number()) {
                ...
                if (edges[j]->get_output_node() != nodes[i]) {
                    ...
                    cerr << "SANITY CHECK FAILED! edges[" << j
                         << "]->output_node had the same innovation number as nodes[" << j
        ...
        if (counted_inputs != number_inputs) {
            cerr << "SANITY CHECK FAILED! nodes[" << i << "] had total inputs: " << number_inputs << " but "
                 << counted_inputs << " inputs were counted. " << endl;
...
bool CNN_Genome::visit_nodes() {
    ...
    for (uint32_t i = 0; i < edges.size(); i++) {
        ...
        // edges[i]->is_forward_visited() << ", is reverse visited: " << edges[i]->is_reverse_visited() << ", is
        // reachable: " << edges[i]->is_reachable() << endl;
...
void CNN_Genome::evaluate_images(const ImagesInterface &images, const vector<int> &batch,
    ...
    for (int32_t batch_number = 0; batch_number < batch.size(); batch_number++) {
        ...
        for (int32_t i = 0; i < (int32_t) softmax_nodes.size(); i++) {
            ...
            // cout << "\tvalue " << softmax_nodes[i]->get_innovation_number() << ": " <<
            // softmax_nodes[i]->get_value_in(0,0) << endl;
        ...
        for (int32_t i = 0; i < (int32_t) softmax_nodes.size(); i++) {
            ...
            // if (training) cout << "\t" << softmax_nodes[i]->get_innovation_number() << " -- batch number: " <<
            // batch_number << ", value: " << value << ", error: " << error << ", gradient: " << gradient << endl;
...
void calculate_softmax(const vector<float> &values_in, vector<float> &values_out, vector<float> &gradient,
    ...
    if (softmax_sum == 0 || isinf(softmax_sum) || isnan(softmax_sum)) {
        ...
        for (uint32_t i = 0; i < values_in.size(); i++) {
            cerr << "\tvalues_in[" << i << "]: " << values_in[i] << ", values_out[" << i << "]: " << values_out[i]
                 << endl;
        }
        exit(1);
    }
    ...
    for (uint32_t i = 0; i < values_in.size(); i++) {
        ...
        // << gradient[i]; gradient[i] += beta * values_out[i] * (-log(values_out[i]) - entropy); cout << ", after
        // penalty: " << gradient[i] << endl;
    ...
    for (int32_t batch_number = 0; batch_number < batch.size(); batch_number++) {
        ...
        */
        ...
        for (uint32_t j = 0; j < softmax_nodes.size(); j++) {
            ...
            cerr << "ag[" << j << "]: " << fixed << setw(11) << setprecision(7) << analytic_softmax_gradient[j]
                 << ", ng[" << j << "]: " << fixed << setw(11) << setprecision(7) << numeric_softmax_gradient
                 << ", re: " << fixed << setw(11) << setprecision(7) << relative_error << endl;
        }
    }
    ...
    for (int32_t i = 0; i < edges.size(); i++) {
        if (edges[i]->is_reachable() && edges[i]->get_type() == CONVOLUTIONAL) {
            ...
            for (int32_t j = 0; j < edges[i]->get_filter_size(); j++) {
                ...
                cerr << "edge[" << i << "], weight[" << j << "]: ae: " << fixed << setw(11) << setprecision(7)
                     << analytic_error << ", ce: " << fixed << setw(11) << setprecision(7) << check_error
                     << ", ne1: " << fixed << setw(11) << setprecision(7) << numeric_error1
                     << ", ne2: " << numeric_error2 << ", ng: " << fixed << setw(11) << setprecision(7)
                     << numeric_gradient << ", ag: " << fixed << setw(11) << setprecision(7) << analytic_gradient
                     << ", relative_error: " << fixed << setw(11) << setprecision(7) << relative_error << endl;
            }
        }
    }
}
    ...
    if (reset_weights) {
        ...
        for (uint32_t i = 0; i < nodes.size(); i++) {
            if (nodes[i]->is_reachable() && nodes[i]->needs_init()) {
                ...
                // cout << "not initializing nodes[" << i << "], reachable: " << nodes[i]->is_reachable() << ", needs
                // init: " << nodes[i]->needs_init() << endl;
            }
        }
        // cout << "initialized node gamma/beta!" << endl;
        ...
        for (uint32_t i = 0; i < edges.size(); i++) {
            if (edges[i]->has_zero_weight()) {
                cerr << "ERROR: edge in position " << i << " with innovation number: " <<
        edges[i]->get_innovation_number() << endl; cerr << "sum of weights was 0" << endl; cerr << "filter_x: " <<
        edges[i]->get_filter_x() << ", filter_y: " << edges[i]->get_filter_y() << endl; exit(1);
            }
        }
        */
    }
...
void CNN_Genome::evaluate(const ImagesInterface &images, const vector<long> &order, float &total_error,
    ...
    for (uint32_t j = 0; j < order.size(); j += batch_size) {
        ...
        float batch_total_error = 0.0;
        int batch_correct_predictions = 0;
        evaluate_images(images, batch, training, batch_total_error, batch_correct_predictions,
                        accumulate_test_statistics);
...
void CNN_Genome::read(istream &infile) {
    ...
    if (line.compare("INNOVATION_NUMBERS") != 0) {
        cerr << "ERROR: invalid input file, expected line to be 'INNOVATION_NUMBERS' but line was '" << line << "'"
             << endl;
        version_str = "INALID";
        return;
    }
...
bool CNN_Genome::is_identical(CNN_Genome *other, bool testing_checkpoint) {
    ...
    if (are_different("backprop_order", backprop_order, other->backprop_order)) {
        if (testing_checkpoint) {
            ...
            cerr << "backprop_order is different, however from CNNs stored in a database they are not required to be "
                    "the same."
                 << endl;
        }
    }
