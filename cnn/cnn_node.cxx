...
void CNN_Node::export_to_database(int _exact_id, int _genome_id) {
  ...
  query << " exact_id = " << exact_id << ", genome_id = " << genome_id << ", innovation_number = " << innovation_number
        << ", depth = " << depth << ", batch_size = " << batch_size << ", size_x = " << size_x
        << ", size_y = " << size_y << ", type = " << type << ", forward_visited = " << forward_visited
        << ", reverse_visited = " << reverse_visited << ", weight_count = " << weight_count
        << ", needs_initialization = " << needs_initialization << ", disabled = " << disabled
...
void CNN_Node::apply_dropout(float *values, float *gradients, bool perform_dropout, bool accumulate_test_statistics,
  ...
  if (perform_dropout && !accumulate_test_statistics) {
    for (int32_t current = 0; current < total_size; current++) {
      // cout << "values for node " << innovation_number << " now " << values[batch_number][y][x] << " after adding
      // bias: " << bias[batch_number][y][x] << endl;
...
void CNN_Node::backpropagate_batch_normalization(bool training, float mu, float learning_rate, float epsilon) {
  ...
  for (int32_t current = 0; current < total_size; current++) {
    ...
    errors_in[current] =
        (delta_out * inverse_variance) + (derr_dvariance * inv_m_x_2 * (value_in - batch_mean)) + (derr_dmean * inv_m);
...
void CNN_Node::set_values(const ImagesInterface &images, const vector<int> &batch, int channel, bool perform_dropout,
  ...
  if (batch.size() > batch_size) {
    ostringstream error_message;
    error_message << "ERROR: number of batch images: " << batch.size() << " > batch_size of input node: " << batch_size
                  << endl;
    throw runtime_error(error_message.str());
  }
  ...
  if (input_dropout_probability > 0)
    apply_dropout(values_out, relu_gradients, perform_dropout, accumulate_test_statistics, input_dropout_probability,
                  generator);
}
...
void CNN_Node::output_fired(bool training, float mu, float learning_rate, float epsilon) {
  ...
  // cout << "output fired on node: " << innovation_number << ", outputs fired: " << outputs_fired << ", total_outputs:
  // " << total_outputs << endl;
...
void CNN_Node::print_statistics(const float *values, const float *errors, const float *gradients) {
  ...
  cerr << ", gradient_min: " << gradient_min << ", gradient_avg: " << gradient_avg << ", gradient_max: " << gradient_max
       << endl;
